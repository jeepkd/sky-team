...
import type { GameEvent, GameState, PlaceAction, Role, SlotDef } from './types';
import { buildSlots } from './slots';
import { resolvePlacement, endOfRound } from './resolve';
...
export function validatePlacement(
  ...
  // Coffee: each token spent shifts the die by ±1 (clamped 1–6, no 1↔6 wrap).
...
function nextTurn(state: GameState): Role {
  ...
  // Keep placing with the player who still has dice (the other may have run out).
...
export function applyPlacement(
  ...
  // Resolve this die's immediate effect.
  ...
  // A crash during resolution ends the game immediately.
  if (s.phase === 'ENDED') return { state: s, events };

  // End of round once every die is placed.
